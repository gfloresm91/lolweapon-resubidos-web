...
En móvil vertical los players se apilan y se intercambian sin desmontar los iframes. El chat permanece montado, oculto inicialmente, y al abrirse cubre siempre el segundo player mientras la distribución cambia a 30 % para el player principal y 70 % para el chat. En tablet vertical se usa una proporción más equilibrada de 58/42 sin chat y 42/58 con chat. En orientación horizontal ambos players ocupan una única fila; al mostrar el chat, este cubre solamente el área del player secundario, que permanece montado debajo. Así se evita el espacio vacío causado por combinar una cuadrícula horizontal con las filas 30/70 del modo vertical. La información se mantiene en un modal móvil.

En tablets horizontales de mayor tamaño se conserva la misma interacción: player principal a la izquierda y una columna secundaria de al menos 400 px. Ambos players son visibles inicialmente y el botón de chat cubre únicamente la columna secundaria; el iframe del player cubierto permanece montado. La ficha del canal se oculta en ese rango para priorizar altura útil. El layout compacto llega hasta 1023 px; desde 1024 px se utiliza la vista de escritorio basándose en el ancho disponible, sin exigir `pointer: fine`, para soportar laptops táctiles y la emulación responsive de DevTools. En desktop existen una posición principal y otra secundaria: intercambiar mueve Twitch a la posición de VK y VK a la de Twitch, mientras el chat conserva su columna propia y permanece visible. No existe modal de información en desktop: la ficha pública completa y sus acciones quedan bajo el player principal. El ancho de la columna secundaria se ajusta entre 420 y 600 px y se conserva en el navegador. La vista completa utiliza el ancho elegido previamente, pero oculta el slider para mantener los controles alineados a la derecha y evitar que el control cambie de posición mientras modifica su propio contenedor. La fila exterior de Twitch reserva al menos 340 px: descontando su cabecera y bordes, el iframe mantiene aproximadamente 304 px de alto, con margen sobre los 300 px mínimos exigidos por Twitch. El player principal continúa llenando el panel disponible; no se fuerza a 16:9 porque el control de ancho ya permite equilibrar el teatro sin reducir artificialmente su superficie.

La ayuda es contextual: teléfonos y tablets conservan las instrucciones específicas para VK, mientras desktop muestra los atajos `F` (vista completa), `I` (intercambiar), `Shift+R` (recargar), `?` (ayuda) y `Esc` (cerrar/salir). Los atajos no pueden capturarse mientras el foco permanezca dentro de un iframe cross-origin; basta volver a enfocar la página. Las cabeceras informan además cuándo VK terminó de cargar y la ficha del canal diferencia una actualización correcta de un fallo prescindible del snapshot.
...
Chrome Android puede solicitar “Sitio para ordenadores” para mejorar la compatibilidad con VK y reportar un viewport cercano a 980 px. La página detecta automáticamente teléfonos mediante puntero grueso, soporte táctil y lado físico corto de hasta 600 px; en ese caso activa internamente el layout móvil escalado sin exigir `?layout=android`. El parámetro se conserva como fallback manual. La escala usa el ancho físico vertical u horizontal correspondiente para no sobredimensionar la cabecera al rotar. Como el `transform` móvil convierte al body en referencia de los elementos fijos, la geometría del iframe del chat se divide por esa escala antes de posicionarlo; esto evita aplicar el factor dos veces y desplazar el chat fuera de pantalla. La detección se recalcula al rotar, redimensionar, entrar en fullscreen o cambiar el viewport visual. El iframe fijo del chat permanece oculto por defecto también bajo este layout y solo se habilita junto con el estado visual del botón. El fullscreen móvil usa players apilados en vertical; en horizontal reutiliza el mismo layout y la misma barra integrada del modo normal, incluido el botón de fullscreen que pasa a funcionar como salida. El chat continúa cubriendo solo el player secundario y puede abrirse o cerrarse desde un control propio de la barra fullscreen vertical, sincronizado con el botón exterior. El botón de vista completa se expone también en móvil y depende de una interacción explícita del usuario, como exige la API Fullscreen del navegador. En orientación horizontal, el iframe del chat usa una superficie interna al 125% reducida visualmente al 80%, lo que muestra más contenido sin intentar modificar el documento cross-origin de Twitch ni alterar las vistas vertical y de escritorio.

En teléfonos horizontales, la cabecera global se elimina y sus acciones se trasladan a la cabecera del player principal. El ancho del panel principal se calcula desde su altura útil para que el frame conserve una proporción exacta de 16:9; el espacio restante pertenece al player secundario o al chat. En el teléfono de referencia el resultado se aproxima a 60/40, pero se adapta a las barras y al viewport real para no introducir franjas laterales por un contenedor excesivamente ancho. Al mostrar el chat también se oculta su cabecera externa redundante, de modo que el iframe oficial ocupa toda la altura de la columna derecha. Al intercambiar los players, la barra de acciones y el cálculo 16:9 acompañan al nuevo principal; al volver a vertical se restauran automáticamente en la cabecera global sin remontar los reproductores. La misma medición y estructura se aplican al fullscreen horizontal móvil. Cuando el teclado reduce el viewport en más de 22 %, el player deja de encogerse al llegar al 35 % del ancho y el chat recibe el 65 % restante; al cerrar el teclado vuelve automáticamente al cálculo 16:9 exacto.

En teléfonos verticales con el chat activo, el frame del player principal conserva una proporción 16:9 y el chat recibe la altura restante. La división vertical resultante es dinámica —aproximadamente 42/58 en un teléfono con el área visible de referencia— y acompaña al nuevo principal cuando se intercambian los players. Sin chat se mantiene la distribución vertical general.

La vista completa propia solicita fullscreen para el documento y no para un iframe cross-origin. Mantiene el mismo ancho lateral configurable del modo normal, con el player secundario arriba y el chat abajo, sin remontar ninguno de los tres embeds. En desktop los controles pasan a la cabecera del player principal para recuperar la fila superior sin superponer Twitch ni el chat; al intercambiar, acompañan al nuevo principal. Mientras esta vista está activa se oculta la salida general hacia Inicio y queda una sola acción para abandonar el fullscreen. En móvil se conserva la barra y el comportamiento específico ya descrito. El fullscreen nativo de VK o Twitch continúa mostrando exclusivamente ese proveedor.

La página no resuelve sesión o permisos, no consulta PostgreSQL, no abre WebSockets propios y no hace llamadas dinámicas a Next.js. Nginx entrega el archivo antes del proxy hacia Next.js. La metadata pública de Twitch se publica cada 30 segundos como `/directo-status.json`: `server.mjs` actualiza atómicamente una única copia usando la misma caché compartida de `/api/twitch/status`, y cada navegador solo descarga ese JSON estático cacheable. Si Node, Twitch o el snapshot fallan, los embeds siguen funcionando y la ficha degrada a `No disponible` con una aclaración explícita de que los reproductores siguen operativos.
