...
Chrome Android puede solicitar “Sitio para ordenadores” para mejorar la compatibilidad con VK y reportar un viewport cercano a 980 px. La página detecta automáticamente teléfonos mediante puntero grueso, soporte táctil y lado físico corto de hasta 600 px; en ese caso activa internamente el layout móvil escalado sin exigir `?layout=android`. El parámetro se conserva como fallback manual. La detección se recalcula al rotar, redimensionar, entrar en fullscreen o cambiar el viewport visual. El fullscreen móvil usa players apilados en vertical y dos columnas en horizontal; el chat continúa cubriendo solo el player secundario.
...
- Twitch inicia silenciado para facilitar autoplay y solicita `play()` al montar, al recibir `PAUSE` y al recuperar foco/visibilidad. Ante `PLAYBACK_BLOCKED` realiza dos reintentos limitados, a 300 ms y 1 segundo, que se cancelan al confirmar reproducción u offline; si el navegador exige un gesto, conserva la acción manual `Activar Twitch`. Después de inicializar no vuelve a imponer `setMuted(true)`: si el usuario activa el sonido, los intentos automáticos de reanudar respetan esa decisión. Las restricciones del navegador o del proveedor no se pueden eludir ni garantizan que una view sea contabilizada.
