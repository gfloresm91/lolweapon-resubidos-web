...
| `/directo` | Modo dual público y aislado. Nginx debe servir directamente `public/directo/index.html` antes del proxy a Next; el fallback de Next redirige a `/directo/index.html` mientras esa configuración no esté activa. Contiene solo VK, companion Twitch silenciado, chat y controles mínimos; no resuelve sesión/permisos, no usa PostgreSQL ni abre APIs/WebSockets propios. Soporta responsive y `?layout=android`. Ver `docs/architecture/directo-static.md`. |
