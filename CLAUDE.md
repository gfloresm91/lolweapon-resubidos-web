...
- **Restore de producción en local y login** — después de restaurar el dump, una cookie o almacenamiento local anterior puede apuntar a una sesión que ya no coincide con `PlatformSession`. Si el login queda tomado o no reacciona, limpiar los datos del sitio desde DevTools → Application y volver a `/login`.
