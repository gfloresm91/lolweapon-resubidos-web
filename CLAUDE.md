...
- **Tier List de temporada** — `AnimeTierListEntry`/`AnimeTierListTheme` son independientes de `SeasonalAnime` (solo comparten `AnimeSeason`); Animes sincroniza directo desde AniList (sin AnimeSchedule) y Openings/Endings desde AnimeThemes.moe, dependiendo de que Animes ya esté sincronizado. A diferencia del Calendario, las Tier Lists son públicas por regla de código (visitantes pueden armar el tablero sin guardar) y ambos mantenedores admin tienen CRUD completo con soft-delete, no solo overrides. Un item ya rankeado que se oculte/elimine se mantiene marcado en el tablero guardado del usuario.
