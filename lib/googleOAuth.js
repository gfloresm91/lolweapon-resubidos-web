...
const DEFAULT_GOOGLE_REDIRECT_PATH = "/api/auth/google/callback";

export function getGoogleRedirectUri(request, redirectPath = DEFAULT_GOOGLE_REDIRECT_PATH) {
  const requestOrigin = getRequestOrigin(request);
  const hostname = new URL(requestOrigin).hostname.toLowerCase();
  const isDefaultPath = redirectPath === DEFAULT_GOOGLE_REDIRECT_PATH;

  // Non-default paths (mobile routes) always derive from the real request origin - the fixed
  // GOOGLE_AUTH_REDIRECT_URI env var only ever points at the web callback path.
  if (!isDefaultPath || configuredHostnames().includes(hostname)) {
    return new URL(redirectPath, requestOrigin).toString();
  }
  return process.env.GOOGLE_AUTH_REDIRECT_URI || new URL(redirectPath, requestOrigin).toString();
}
...
export function buildGoogleAuthorizeUrl({ request, state, nonce, redirectPath }) {
  const url = new URL(GOOGLE_AUTHORIZE_URL);
  url.searchParams.set("client_id", getClientId());
  url.searchParams.set("redirect_uri", getGoogleRedirectUri(request, redirectPath));
...
export async function exchangeGoogleCode({ request, code, redirectPath }) {
  const response = await fetch(GOOGLE_TOKEN_URL, {
    ...
    body: new URLSearchParams({
      ...
      redirect_uri: getGoogleRedirectUri(request, redirectPath),
    }),
  });
