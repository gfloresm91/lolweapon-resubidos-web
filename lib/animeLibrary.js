...
export function normalizeEditableAnimeMetadataItem(item) {
  return {
...
export async function deleteAnimeMetadataEntry(key) {
  const normalizedKey = normalizeTag(key);
...
export async function buildAnimeLibrary({ includeHidden = false, metadataOverride } = {}) {
  const [lives, settings, metadata] = await Promise.all([
    readLives(),
    readTagSettings(),
    metadataOverride === undefined ? readAnimeMetadata() : metadataOverride,
  ]);
