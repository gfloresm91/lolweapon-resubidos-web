import { readLives, writeLives } from "@/lib/repositories/liveRepository";
import { sortLives } from "@/lib/lives";
import { buildLiveFromTwitchStream, fetchCurrentTwitchStream } from "@/lib/twitch";

export async function upsertTwitchLive(event = {}) {
  const stream = await fetchCurrentTwitchStream({
    broadcasterUserId: event.broadcaster_user_id,
    broadcasterLogin: event.broadcaster_user_login,
  });

  if (!stream && process.env.TWITCH_REQUIRE_ACTIVE_STREAM === "true") {
    return null;
  }
