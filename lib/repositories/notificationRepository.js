import { notifyDevicesForNewlyPublished } from "../mobilePush.js";
import { broadcastNotificationUpdate } from "../notificationRealtime.js";
import { invalidateLiveCatalog } from "../liveCatalogState.js";
...
export async function listNotifications({ user = null, limit = DEFAULT_LIMIT } = {}) {
  const requestedLimit = normalizeLimit(limit, DEFAULT_LIMIT, MAX_MARK_ALL);
  const visibleNotifications = await findVisibleNotifications({ user, limit: MAX_MARK_ALL });
  const unreadCount = visibleNotifications.filter((notification) => !notification.isRead).length;

  return {
    notifications: visibleNotifications.slice(0, requestedLimit),
    unreadCount,
  };
}
...
export async function listUserNotifications({ user, search = "", type = "all", status = "all", page = 1, pageSize = 10, sort = "published", direction = "desc" } = {}) {
  ...
  const sorted = [...filtered].sort((left, right) => {
    ...
    if (sort === "type") {
      ...
      leftValue = left.isDismissed ? 2 : left.isRead ? 1 : 0;
      rightValue = right.isDismissed ? 2 : right.isRead ? 1 : 0;
    } else {
      leftValue = new Date(left.publishedAt || left.createdAt || 0).getTime();
      rightValue = new Date(right.publishedAt || right.createdAt || 0).getTime();
    }

    if (leftValue < rightValue) return -1 * sortDirection;
    if (leftValue > rightValue) return 1 * sortDirection;
    return (Number(left.id) - Number(right.id)) * sortDirection;
  });
  ...
  return {
    notifications: sorted.slice((safePage - 1) * safePageSize, safePage * safePageSize),
    total,
    page: safePage,
...
export async function createPlatformNotification({
  ...
  try {
    ...
    if (notification.publishedAt) {
      ...
      void notifyDevicesForNewlyPublished(notification);
    }
...
export async function createResubidoNotification({ live, actor = null }) {
  ...
  invalidateLiveCatalog({ action: "notification-updated", liveId: live.id });
  void notifyDevicesForNewlyPublished(notification);
