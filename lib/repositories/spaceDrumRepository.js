...
import { getSpaceDrumThumbnailPage } from "../spacedrumThumbnail.js";
...
async function writePostgresSpaceDrum(data, language = DEFAULT_SPACE_DRUM_LANGUAGE) {
  ...
  const incomingLegacyIds = new Set(normalized.chapters.map((chapter) => chapter.id));
  await prisma.spaceDrumChapter.updateMany({
    where: {
      spaceDrumId: saved.id,
      legacyId: { notIn: Array.from(incomingLegacyIds) },
    },
    data: { status: "hidden" },
  });

  for (const [position, chapter] of normalized.chapters.entries()) {
    const savedChapter = await safeUpsert(prisma.spaceDrumChapter, {
      where: {
        spaceDrumId_legacyId: {
          spaceDrumId: saved.id,
          legacyId: chapter.id,
        },
      },
    ...
    await prisma.spaceDrumPage.deleteMany({ where: { chapterId: savedChapter.id } });
