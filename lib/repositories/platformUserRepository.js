...
let platformPermissionsReady = false;
let platformPermissionsPromise = null;
let publicAccessUserCache = null;
let publicAccessUserPromise = null;
const PUBLIC_ACCESS_USER_TTL_MS = 30 * 1000;
...
export const DEFAULT_PLATFORM_PERMISSIONS = [
  ...
  { code: "admin.audience.view", label: "Ver audiencia web", group: "Administración: Audiencia web", description: "Consulta presencia concurrente e historial agregado por directo.", sortOrder: 469 },
...
export async function ensurePlatformPermissions(prisma = getPrismaClient()) {
  if (platformPermissionsReady) return;

  if (!platformPermissionsPromise) {
    platformPermissionsPromise = ensurePlatformPermissionsOnce(prisma)
      .then(() => {
        platformPermissionsReady = true;
      })
      .finally(() => {
        platformPermissionsPromise = null;
      });
  }

  return platformPermissionsPromise;
}

async function ensurePlatformPermissionsOnce(prisma) {
  ...
  await assignDefaultPermissions(prisma, DEFAULT_ROLE_CODE, [
    ANIME_TIER_LIST_ANIMES_PERMISSION_CODE,
    ANIME_TIER_LIST_OPENINGS_PERMISSION_CODE,
  ]);
...
export async function getPublicAccessUser() {
  if (publicAccessUserCache && Date.now() - publicAccessUserCache.updatedAt <= PUBLIC_ACCESS_USER_TTL_MS) {
    return publicAccessUserCache.user;
  }

  if (!publicAccessUserPromise) {
    publicAccessUserPromise = loadPublicAccessUser()
      .then((user) => {
        publicAccessUserCache = { user, updatedAt: Date.now() };
        return user;
      })
      .finally(() => {
        publicAccessUserPromise = null;
      });
  }

  return publicAccessUserPromise;
}

async function loadPublicAccessUser() {
...
/**
 * Borrado de cuenta self-service (mobile "eliminación completa de cuenta"). A diferencia de
 * deletePlatformUser (soft-delete admin, deja email/login intactos): acá se scrubbea el PII y se
 * destruye todo el material de auth (identidades, sesiones, tokens mobile), pero el contenido del
 * usuario (tickets, ratings, tier lists) queda intacto, ahora asociado a un usuario anonimizado -
 * decisión de producto confirmada 2026-08-18, ver docs/backend-api.md del repo mobile.
 */
export async function anonymizeAndDeactivatePlatformUser(id) {
  const prisma = getPrismaClient();
  const userId = Number(id);
  const user = await prisma.platformUser.findUnique({ where: { id: userId }, include: { role: true } });
  ...
  await prisma.$transaction([
    prisma.platformUser.update({
      where: { id: userId },
      data: {
        email: `deleted-${userId}@deleted.lolweapon.internal`,
        login: `deleted-user-${userId}`,
        alias: "Usuario eliminado",
        avatarUrl: null,
        passwordHash: null,
        passwordSalt: null,
    ...
    prisma.platformAuthIdentity.deleteMany({ where: { userId } }),
    prisma.platformSession.deleteMany({ where: { userId } }),
    prisma.platformMobileRefreshToken.deleteMany({ where: { userId } }),
    prisma.platformMobileAccessToken.deleteMany({ where: { userId } }),
    prisma.platformMobileOAuthExchange.deleteMany({ where: { userId } }),
    prisma.platformMobilePushToken.deleteMany({ where: { userId } }),
  ]);
}
...
export async function createPlatformSession(userId) {
  ...
  return { token, expiresAt: session.expiresAt, user: compactUser(user) };
}
