...
const USER_MANAGEMENT_ROLES = new Set(["dios", "admin"]);
...
export const DEFAULT_PLATFORM_PERMISSIONS = [
  ...
  { code: "admin.anime.tracking.view", label: "Ver mantenedor Viendo", group: "Administración: Viendo", sortOrder: 80 },
  { code: "admin.anime.completed.view", label: "Ver mantenedor Terminados", group: "Administración: Terminados", sortOrder: 90 },
  { code: "anime.tracking.view", label: "Ver Viendo", group: "Anime: Viendo", sortOrder: 100 },
  { code: "anime.tracking.create", label: "Crear en Viendo", group: "Anime: Viendo", sortOrder: 110 },
  { code: "anime.tracking.update", label: "Editar Viendo", group: "Anime: Viendo", sortOrder: 120 },
  { code: "anime.tracking.delete", label: "Eliminar de Viendo", group: "Anime: Viendo", sortOrder: 130 },
  { code: "anime.tracking.form.full", label: "Formulario completo", group: "Anime: Viendo", sortOrder: 140 },
  { code: "anime.tracking.form.compact", label: "Formulario compacto", group: "Anime: Viendo", sortOrder: 150 },
  { code: "anime.completed.view", label: "Ver Terminados", group: "Anime: Terminados", sortOrder: 160 },
  { code: "anime.completed.create", label: "Crear en Terminados", group: "Anime: Terminados", sortOrder: 170 },
  { code: "anime.completed.update", label: "Editar Terminados", group: "Anime: Terminados", sortOrder: 180 },
  { code: "anime.completed.delete", label: "Eliminar de Terminados", group: "Anime: Terminados", sortOrder: 190 },
  { code: "anime.completed.form.full", label: "Formulario completo", group: "Anime: Terminados", sortOrder: 200 },
  { code: "anime.completed.form.compact", label: "Formulario compacto", group: "Anime: Terminados", sortOrder: 210 },
...
function compactRole(role) {
  ...
  return {
    ...
    permissions: role.code === GOD_ROLE_CODE ? DEFAULT_PLATFORM_PERMISSIONS.map((permission) => permission.code) : permissions,
  };
}
...
export function userCanManagePlatformUsers(user) {
  return can(user, "users.read") || Boolean(user?.isActive && USER_MANAGEMENT_ROLES.has(user.role));
}

export function userCanManagePlatformRoles(user) {
  return can(user, "roles.read") || Boolean(user?.isActive && USER_MANAGEMENT_ROLES.has(user.role));
}
...
export async function ensurePlatformPermissions(prisma = getPrismaClient()) {
  ...
  const existingPermissions = await prisma.platformPermission.findMany({
    where: { code: { in: DEFAULT_PLATFORM_PERMISSIONS.map((permission) => permission.code) } },
    select: { code: true },
  });
  ...
  for (const permission of DEFAULT_PLATFORM_PERMISSIONS) {
    if (existingPermissionCodes.has(permission.code)) {
      await prisma.platformPermission.update({
...
export async function upsertPlatformRole(input, { actor = null } = {}) {
  ...
  const isCreating = !id;
  const permissionCodes = Array.isArray(input?.permissions) ? input.permissions : [];
...
export async function findOrCreateTwitchUser(profile) {
  ...
  const bootstrapRole = await resolveRole(prisma, BOOTSTRAP_ROLE_CODE);
