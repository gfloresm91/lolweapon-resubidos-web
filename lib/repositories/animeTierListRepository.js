...
async function getThemeRosterForSeason(seasonId, kind) {
  ...
  const rows = await prisma.animeTierListTheme.findMany({
    where: { entry: { seasonId }, removedAt: null },
    include: { entry: true },
  });
  ...
  return rows
    .map((row) => {
      ...
      return {
        ...
        primarySourceLabel: row.manualPrimarySourceLabel || "Fuente principal",
        alternateVideoUrls: Array.isArray(row.alternateVideoUrls) ? row.alternateVideoUrls : [],
        ...
        isEntryHidden: entryHidden,
        isDeletedByAdmin: Boolean(row.deletedAt),
        isDraft: !row.deletedAt && row.manualVisible === false,
        type: row.type,
