...
export async function analyzeTrackerWorkbook(buffer, currentLives, statuses = []) {
  ...
  for (let rowNumber = 2; rowNumber <= sheet.rowCount; rowNumber += 1) {
    ...
    const candidateDiffs = getFieldDiffs(normalizedSnapshot(current), normalizedSnapshot(candidate));
    ...
    if (!validation.success) {
      result.errors.push({ row: rowNumber, dbId, internalId, message: validation.error.issues[0]?.message || "La fila contiene datos inválidos." });
      continue;
    }
    const next = { ...validation.data, dbId };
