...
function themeScore(theme) {
  return (theme.isSpoiler ? 0 : 1) * 1000 + (theme.videoResolution || 0);
}
...
export async function previewAnimeTierListThemeSync({ seasonId }) {
  ...
  const existingAniListIds = new Set(entries.map((entry) => entry.aniListId).filter(Boolean));
...
export async function applyAnimeTierListThemeSync({ seasonId }) {
  ...
  if (season) {
    ...
    const existingAniListIds = new Set(existing.map((entry) => entry.aniListId).filter(Boolean));
    ...
    for (const entry of preview.entries) {
      ...
      for (const theme of themes) {
        ...
        await tx.animeTierListTheme.upsert({
          ...
          update: data,
          create: { tierListEntryId: entry.id, animeThemeId: theme.animeThemeId, ...data },
        });
      }
    }
  }, { timeout: 60000 });
