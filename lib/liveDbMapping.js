...
export async function ensureLiveStatus(prisma, value) {
  ...
  return safeUpsert(prisma.liveStatus, {
    where: { code },
    update: {
...
export async function ensureLinkPlatform(prisma, value) {
  ...
  return safeUpsert(prisma.linkPlatform, {
    where: { code },
    update: {
...
export async function saveLiveRecord(prisma, live, cache = {}) {
  const statusKey = String(live.status || "").trim().toLowerCase();
  let status = cache.statuses?.get(statusKey);
  if (!status) {
    status = await ensureLiveStatus(prisma, live.status);
    cache.statuses?.set(statusKey, status);
  }
  const savedLive = await safeUpsert(prisma.live, {
  ...
  for (const [position, tagName] of (live.tags || []).entries()) {
    ...
    let tag = cache.tags?.get(tagKey);
    if (!tag) {
      tag = await ensureLiveTag(prisma, tagName);
      if (tag) cache.tags?.set(tagKey, tag);
    }
    ...
    liveTags.push({ liveId: savedLive.id, tagId: tag.id, position });
  }
  if (liveTags.length) await prisma.liveTag.createMany({ data: liveTags });

  await prisma.liveLink.deleteMany({ where: { liveId: savedLive.id } });

  const liveLinks = [];
  for (const platformCode of defaultLinkPlatformCodes) {
    let platform = cache.platforms?.get(platformCode);
    if (!platform) {
      platform = await ensureLinkPlatform(prisma, platformCode);
      if (platform) cache.platforms?.set(platformCode, platform);
    }
    const urls = Array.isArray(live.links?.[platformCode]) ? live.links[platformCode] : [];

    for (const [index, url] of urls.entries()) {
      liveLinks.push({ liveId: savedLive.id, platformId: platform.id, url, position: index });
    }
  }
  if (liveLinks.length) await prisma.liveLink.createMany({ data: liveLinks });
}
