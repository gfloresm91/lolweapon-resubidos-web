...
export function getPagePresenceSocketCount() {
  return presence.sockets.size;
}

function flushBroadcastCount() {
  presence.broadcastTimer = null;
  const count = getHomePresenceCount();
  if (count === presence.lastBroadcastCount) return;
  ...
  for (const client of presence.sockets) {
    if (!sendJson(client, payload)) staleClients.push(client);
  }
...
function scheduleBroadcastCount() {
  if (presence.broadcastTimer) return;
  presence.broadcastTimer = setTimeout(flushBroadcastCount, BROADCAST_DEBOUNCE_MS);
  presence.broadcastTimer.unref?.();
}
