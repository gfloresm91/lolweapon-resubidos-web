...
export function getTwitchRedirectUri(request) {
  const requestOrigin = getRequestOrigin(request);
  const requestHostname = new URL(requestOrigin).hostname.toLowerCase();
  const configuredHostnames = getConfiguredHostnames();

  if (configuredHostnames.includes(requestHostname)) {
    return new URL("/api/auth/twitch/callback", requestOrigin).toString();
  }

  return process.env.TWITCH_AUTH_REDIRECT_URI || new URL("/api/auth/twitch/callback", requestOrigin).toString();
}
...
export function buildTwitchAuthorizeUrl({ request, state }) {
...
export async function exchangeTwitchCode({ request, code }) {
