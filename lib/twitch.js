...
async function listStreamOnlineSubscriptions(token) {
  const subscriptions = [];
  let cursor = null;

  do {
    const params = new URLSearchParams({ type: "stream.online" });
...
function matchesStreamOnlineSubscription(subscription, broadcasterId) {
  return subscription.type === "stream.online"
...
export async function ensureStreamOnlineSubscription() {
  ...
  const broadcaster = await fetchTwitchBroadcaster();
  const callback = getRequiredEnv("TWITCH_EVENTSUB_CALLBACK_URL");
  const subscriptions = await listStreamOnlineSubscriptions(token);
  const matchingSubscriptions = subscriptions.filter((subscription) => (
    matchesStreamOnlineSubscription(subscription, broadcaster.id)
  ));
