...
import { getPlatformUserById } from "./repositories/platformUserRepository.js";
...
/**
 * Resuelve la identidad móvil desde el bearer. Los endpoints públicos deben declarar su acceso
 * directamente en el handler; este helper solo devuelve usuarios autenticados.
 */
export async function getMobileAccessUser(request) {
  const userId = await getMobileUserIdFromRequest(request);
  if (!userId) return null;

  return getPlatformUserById(userId);
}
