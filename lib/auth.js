...
export const SESSION_COOKIE = /^[A-Za-z0-9!#$%&'*+.^_`|~-]+$/.test(configuredSessionCookie)
...
function getConfiguredSessionCookieDomain(request) {
  ...
  const requestHost = forwardedHost || request?.headers.get("host") || (request ? new URL(request.url).host : "");
